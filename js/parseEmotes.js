...
async function parseEmotes(message, messageEmotes = {}) {
  await getEmoteRegex();
  let parsedMessage = '';
  if (messageEmotes) {
    ...
    const emoteStart = emoteIds.reduce((starts, id) => {
      messageEmotes[id].forEach((startEnd) => {
        ...
        starts[start] = {
          url: `<img class="cheer-mote" src="https://static-cdn.jtvnw.net/emoticons/v2/${id}/default/dark/4.0#emote">`,
          end,
        };
      });
      return starts;
  ...
  const result = (parsedMessage || message)
    .replace(emoteRegex, (code) => `<img class="cheer-mote" src="${emotes[code.toLowerCase()]}">`);
